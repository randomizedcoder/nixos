# pi-sebastian - Raspberry Pi 4 NixOS demo card

A NixOS SD-card image for a Raspberry Pi 4B, built for **Sebastian** to try
NixOS on his Pi 4. It is the Pi-4 sibling of the `pi-bob` card (same features
and modular layout, just targeting Pi 4 hardware). Pi-specific support (kernel,
...
The Raspberry Pi 4 needs a solid **5 V at 3 A (15 W)** USB-C supply. Use the
**official Raspberry Pi 15 W USB-C power supply** (5.1 V / 3 A) or an equivalent
quality 5 V/3 A adapter, plugged **straight into the wall** with a good cable.

An underpowered supply causes **undervoltage** brownouts under CPU + I/O load:
the board throttles and, at worst, hard-hangs during heavy work like a
`nixos-rebuild`. We hit exactly that on the Pi 5 sibling (`pi-bob`) - it looked
like an SD-card or NixOS bug, but the real cause was the 5 V rail sagging under
load. A proper supply fixed it; no amount of I/O tuning fixes a volts problem.
So don't run it off a weak phone charger or a shared multi-port hub.

Note: unlike the Pi 5 (which needs **5 V/5 A / 25 W** negotiated over USB-C PD),
the Pi 4 only needs **5 V/3 A** - so many bricks that are inadequate for a Pi 5
are perfectly fine for a Pi 4, as long as they genuinely hold 5 V at 3 A.

Quick check on the Pi (non-zero = the supply isn't keeping up):

```sh
vcgencmd get_throttled   # 0x0 = ok; bit0 (0x1)=undervoltage now; bit16 (0x10000)=since boot
```

(The Pi 4 has no PMIC voltage ADC, so `vcgencmd pmic_read_adc` - handy on the
Pi 5 - isn't available here; `get_throttled` is the check. The config also
bounds the rebuild's memory/IO pressure via `nix/io-throttle.nix`, which lowers
peak load, but a proper PSU is the real fix.)
...
cd ~/nixos/arm/pi-sebastian
nix build .#nixosConfigurations.pi-sebastian-sdimage.config.system.build.sdImage
...
`ssh bob@pi-sebastian.local` (password `bob`). The root partition auto-grows to fill
...
so `pi-sebastian.local` should resolve on the LAN.
...
```sh
ssh bob@pi-sebastian.local          # password: bob
ssh sebastian@pi-sebastian.local    # password: sebastian
```

If `pi-sebastian.local` does not resolve, find the Pi's IP from the console
...
```sh
cd ~/nixos/arm/pi-sebastian
sudo nixos-rebuild switch --flake .#pi-sebastian
```
...
iperf2 -c pi-sebastian.local -p 5001 --permit-key=pleaseChangeMe  # from another host
```

The full C/autotools toolchain (`gcc`, `autoconf`, `automake`, `libtool`,
`pkg-config`, `gdb`, `valgrind`, ...) is installed so you can build and run
iperf2 from source too.

## 7. Network performance tuning (dedicated NIC core)

This card runs an iperf2 server, so the Pi is tuned to give **clean, repeatable
network numbers** rather than leaving throughput at the mercy of whatever else
the box is doing. See `nix/net-tuning.nix`.

The Pi 4's on-board 1 GbE (`eth0`, the on-SoC bcmgenet MAC) is effectively a
**single-queue NIC**: its interrupt is serviced on **CPU0** by default - right
next to the SD-card controller. Under any other load that core becomes the
bottleneck and results get noisy. (If `ip -brief link` shows the wired port
under a different name, set `nic` in `nix/net-tuning.nix` accordingly.)
...
```sh
grep eth0 /proc/interrupts                     # which CPU column is counting
taskset -cp "$(pgrep -x iperf2 | head -1)"     # iperf2 server -> CPU 3
cat /sys/class/net/eth0/queues/rx-0/rps_cpus   # RPS mask -> 8 (i.e. CPU3)
```
...
```
http://pi-sebastian.local:3000        (or http://<pi-ip>:3000)
```
...
own query UI is at `http://pi-sebastian.local:9090`.
...
A **writable copy of this config ships on the Pi** at `~/pi-sebastian` (seeded
into sebastian's home on first boot). Sebastian can edit it directly on the Pi
and rebuild - no laptop or re-flash needed:
...
only created if `~/pi-sebastian` doesn't exist, so edits survive future rebuilds; to
start over, `rm -rf ~/pi-sebastian` and rebuild once to re-seed a pristine copy.
...
```sh
cd ~/nixos/arm/pi-sebastian
nixos-rebuild switch --flake .#pi-sebastian --target-host root@pi-sebastian.local
```

This builds `pi-sebastian` for aarch64. If your PC is x86_64 it needs aarch64
...
The `pi-sebastian-cross` output is the exact same system as `pi-sebastian`, but
...
```sh
cd ~/nixos/arm/pi-sebastian
nixos-rebuild switch --flake .#pi-sebastian-cross --target-host root@pi-sebastian.local
```
...
The SD image (`pi-sebastian-sdimage`, §1) is cross-compiled the same way, so a freshly
...
```sh
cd ~/pi-sebastian                 # on the Pi (or ~/nixos/arm/pi-sebastian on your PC)
nix flake update           # rewrite flake.lock to the latest inputs
sudo nixos-rebuild switch --flake ~/pi-sebastian#pi-sebastian        # rebuild on the Pi
...
- `flake.nix` - inputs and three outputs: `pi-sebastian` (native deploy config),
  `pi-sebastian-cross` (same system cross-compiled from x86_64 for fast PC builds,
  §11), and `pi-sebastian-sdimage` (the flashable image); wires home-manager for all
  three users.
...
- `nix/seed-config.nix` - drops a writable copy of this config into
  `/home/sebastian/pi-sebastian` on first boot (so Sebastian can edit + rebuild
  on the Pi).
