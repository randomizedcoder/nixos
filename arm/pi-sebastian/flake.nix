#
# arm/pi-sebastian/flake.nix
#
# NixOS config for a Raspberry Pi 4 SD-card image for Sebastian, so he can try
# NixOS on his Pi 4. It is the Pi-4 sibling of ../pi-bob (same features, same
# modular layout), differing only in the hardware target. Pi-specific support
# (kernel, firmware, bootloader, vendor pkgs) comes from nixos-raspberrypi.
# SD-card-only (stock kernel), split into small ./nix/ modules.
#
...
#   - pi-sebastian          the running system (native aarch64 deploy target).
#   - pi-sebastian-cross    the same system, cross-compiled from x86_64 for fast PC
#                     builds you push to the Pi (no on-Pi/QEMU compiling).
#   - pi-sebastian-sdimage  the same system, packaged as a flashable SD-card image
...
#    is seeded to ~/pi-sebastian on first boot (see nix/seed-config.nix), so on the Pi:
...
#       nix build .#nixosConfigurations.pi-sebastian.config.system.build.toplevel
#   - Update the flake inputs (e.g. newer kernel):       nix flake update
#
{
  description = "pi-sebastian - Raspberry Pi 4 NixOS demo card for Sebastian";
  ...
  outputs =
    ...
    in
    {
      nixosConfigurations = {
        ...
        pi-sebastian = nixos-raspberrypi.lib.nixosSystem {
          specialArgs = inputs;
          modules = baseModules;
        };

        # The same running system as pi-sebastian, but cross-compiled from x86_64 so
        ...
        pi-sebastian-cross = nixos-raspberrypi.lib.nixosSystem {
          specialArgs = inputs;
          modules = baseModules ++ [
        ...
        #   nix build .#nixosConfigurations.pi-sebastian-sdimage.config.system.build.sdImage
        pi-sebastian-sdimage = nixos-raspberrypi.lib.nixosSystem {
          specialArgs = inputs;
          modules = baseModules ++ [
