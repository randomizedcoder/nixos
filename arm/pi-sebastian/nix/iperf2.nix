#
# arm/pi-sebastian/nix/iperf2.nix
#
# iperf2 network-test server, supervised by systemd. Ported from
# ~/nixos/hp/hp4/iperf2.nix - the same service the pi-bob card runs (Bob is the
# iperf2 maintainer; this Pi-4 sibling ships the same setup for Sebastian).
#
...
in
{
  systemd.services.iperf2 = {
    ...
    serviceConfig = {
      ...
      #   iperf2 -c pi-sebastian.local -p 5001 --permit-key=pleaseChangeMe
  ...
  # UDP. On pi-sebastian the NixOS firewall is enabled by default, so these openings
