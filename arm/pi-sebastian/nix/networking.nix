#
# arm/pi-sebastian/nix/networking.nix
#
...
# mDNS via avahi makes `pi-sebastian.local` resolve on the LAN.
#
...
{
  networking.hostName = "pi-sebastian";
  ...
  # mDNS so `pi-sebastian.local` resolves on the LAN.
  services.avahi = {
    enable = true;
    nssmdns4 = true;
    ipv4 = true;
