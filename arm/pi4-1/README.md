# pi4-1 - Raspberry Pi 4

NixOS config for the Raspberry Pi 4 "pi4-1". Pi-specific support (kernel,
...
```sh
ssh root@pi4-1.local
# or
ssh das@pi4-1.local
```

mDNS is enabled, so `pi4-1.local` should resolve on the LAN. If it does not,
...
```sh
cd ~/nixos/arm/pi4-1
nixos-rebuild switch --flake .#pi4-1 --target-host root@pi4-1.local
```

## Files

- `flake.nix` - flake inputs and the two outputs (`pi4-1` deploy config,
  `pi4-1-sdimage` image build).
