...

{
  ...
  # Pi 4 uses u-boot (the raspberry-pi-4.base default).
  ...
  # linux_rpi4 (Cortex-A72 6.12.87). Same shape as the Pi 5 hosts.
  # Default sysctl net.core.flow_dissector_fastpath=0; revert to
  # stock by removing this block.
  ...
  # mDNS so `pi4-1.local` resolves on the LAN (the installer had this off).
  services.avahi = {
    enable = true;
    nssmdns4 = true;
    ipv4 = true;
