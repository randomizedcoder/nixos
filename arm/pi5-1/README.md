# pi5-1 - Raspberry Pi 5

NixOS config for the Raspberry Pi 5 "pi5-1". Pi-specific support (kernel,
...
```sh
ssh root@pi5-1.local
# or
ssh das@pi5-1.local
```

mDNS is enabled, so `pi5-1.local` should resolve on the LAN. If it does not,
...
```sh
cd ~/nixos/arm/pi5-1
nixos-rebuild switch --flake .#pi5-1 --target-host root@pi5-1.local
```

## Files

- `flake.nix` - flake inputs and the two outputs (`pi5-1` deploy config,
  `pi5-1-sdimage` image build).
