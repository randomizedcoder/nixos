# pi5-2 - Raspberry Pi 5

NixOS config for the Raspberry Pi 5 "pi5-2". Pi-specific support (kernel,
...
```sh
cd ~/nixos/arm/pi5-2
nix build .#nixosConfigurations.pi5-2-sdimage.config.system.build.sdImage
```
...
```sh
ssh root@pi5-2.local
# or
ssh das@pi5-2.local
```

mDNS is enabled, so `pi5-2.local` should resolve on the LAN. If it does not,
...
```sh
cd ~/nixos/arm/pi5-2
nixos-rebuild switch --flake .#pi5-2 --target-host root@pi5-2.local
```

## Files

- `flake.nix` - flake inputs and the two outputs (`pi5-2` deploy config,
  `pi5-2-sdimage` image build).
