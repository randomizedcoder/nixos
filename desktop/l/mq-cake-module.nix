...
#   git add patches/mq-cake-6.18.8/*.patch
#
# ============================================================================
#
...
let
  # Use 6.18.8-adapted patches - fixed for actual 6.18.8 kernel API
  patchDir = ./patches/mq-cake-6.18.8;

  mqCakePatches = [
    {
      name = "mq-cake-01-export-mq-functions";
      patch = "${patchDir}/01-export-mq-functions.patch";
    }
    {
      name = "mq-cake-02-factor-out-config";
      patch = "${patchDir}/02-factor-out-config.patch";
    }
    {
      name = "mq-cake-03-add-cake_mq";
      patch = "${patchDir}/03-add-cake_mq.patch";
    }
    {
      name = "mq-cake-04-share-config";
      patch = "${patchDir}/04-share-config.patch";
    }
    {
      name = "mq-cake-05-share-shaper-state";
      patch = "${patchDir}/05-share-shaper-state.patch";
    }
  ];
