# ~/nixos/super/a/networking.nix   —  node a (nodeA)
#
...
# multi-chassis LAG. The switch server ports are TRUNKS (Nexus vPC 217,
...
{ lib, ... }:
{
  ...
  networking.hostName = lib.mkForce "nodeA";
  ...
  # --- LACP bond -> Nexus vPC 217 (trunk, tagged VLAN 400-401) ---
  ...
  systemd.network.networks."30-bond-member-lan1" = {
    matchConfig.PermanentMACAddress = "ac:1f:6b:15:d8:80";  # node a  System LAN1
    networkConfig.Bond = "bond0";
  };
  systemd.network.networks."30-bond-member-lan2" = {
    matchConfig.PermanentMACAddress = "ac:1f:6b:15:d8:81";  # node a  System LAN2
    networkConfig.Bond = "bond0";
  };
  ...
  systemd.network.networks."45-bond0.401" = {
    ...
    address = [ "10.241.10.10/24" ];   # node a
